                                   ...
                                   <table class="table">
                                    ...
                                    <tbody>
                                        ...
                                        <tr class="user-row" id="blog-{{ $blog->id }}">
                                               ...
                                               <td id="image">
                                                <img height="80" width="80" src="{{ asset('images/' . $blog->sub_image) }}"/>
                                                </td>
                                               <td id="email">{{$blog->heading}}</td>  
                                               <td id="number">{{$blog->paragraph}}</td>    
                                                                                              
                                                <td>
                                                    <a id="blogedit" data-blog-id="{{ $blog->id }}" class="btn btn-warning mx-5 edit-blog-btn">
                                                ...
                                                <td>
                                                    <a data-blog-id="{{ $blog->id }}" class="btn btn-danger delblog">
                ...
                Swal.fire({
                    ...
                    const newRow = `
                        <tr data-blog-id="${blog.id}">
                            ...
                            <td><img height="80" width="80" src="{{ asset('images/') }}/${blog.sub_image}" /></td>
                            <td>${blog.heading}</td>
     ...
     $.ajax({
        ...
        success: function (response) {
            if (response.success) {
                ...
                if (response.blog.sub_image) {
                    $('#blogeditform #subimage_edit').attr('src', "{{ asset('images') }}/" + response.blog.sub_image);
                }
        ...
        success: function (response) {
            ...
            if (response.success) {
                Swal.fire({
                    ...
                    blog.find('td:nth-child(2) img').attr('src', `/images/${response.blog.sub_image}`);
                    blog.find('td:nth-child(3)').text(response.blog.heading); 
                    blog.find('td:nth-child(4)').text(response.blog.paragraph); 
                });
            }
        },
        error: function (xhr) {
